...
{
    $modules = mainShop()?->shopCategory?->businessModules;
    if (mainShop()?->businessModules) {
        $modules = mainShop()->businessModules;
    }
    return $modules && $modules->where('name', $feature)->first() ? true : false;
}
